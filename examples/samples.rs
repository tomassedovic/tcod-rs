...
trait Render {
    ...
    fn render(&mut self, console: &mut Offscreen, root: &Root, event: Option<(EventFlags, Event)>);
}
...
impl ColorsSample {
    ...
    fn cycle_color_component(&self, color_component: u8, dir: i8) -> (u8, i8) {
        ...
        let new_dir = if new_component == 255 {
            -1
    ...
    fn set_colors(&self, console: &mut dyn Console) {
        ...
        for x in 0..SAMPLE_SCREEN_WIDTH {
            ...
            let top = colors::lerp(
                self.cols[Dir::TopLeft as usize],
                self.cols[Dir::TopRight as usize],
                xcoef,
            );
            let bottom = colors::lerp(
                self.cols[Dir::BottomLeft as usize],
...
impl Render for OffscreenSample {
    fn initialize(&mut self, console: &mut Offscreen) {
        if !self.init {
            ...
            self.secondary.print_frame(
                ...
                false,
                BackgroundFlag::Set,
                Some("Offscreen console"),
            );
            self.secondary.print_rect_ex(
    ...
    fn render(
        ...
        if self.counter % 20 == 0 {
            self.x += self.xdir;
            self.y += self.ydir;
            if self.x == (SAMPLE_SCREEN_WIDTH / 2 + 5) {
                self.xdir = -1
            } else if self.x == -5 {
                self.xdir = 1
            }
            if self.y == (SAMPLE_SCREEN_HEIGHT / 2 + 5) {
                self.ydir = -1
            } else if self.y == -5 {
                self.ydir = 1
            }
...
impl LineSample {
    ...
    fn next_flag(&mut self, flag_byte: i32) {
        ...
        if flag_byte >= max - 1 {
            ...
            self.bk_flag = unsafe { std::mem::transmute(flag_byte + 1) }
        }
    }
...
impl Render for LineSample {
    ...
    fn render(
        ...
        let rect_y =
            ((SAMPLE_SCREEN_HEIGHT - 2) as f32 * ((1.0 + elapsed_seconds.cos()) / 2.0)) as i32;
        for x in 0..SAMPLE_SCREEN_WIDTH {
        ...
        let yo = ((SAMPLE_SCREEN_HEIGHT / 2) as f32 + sin_angle * (SAMPLE_SCREEN_WIDTH / 2) as f32)
            as i32;
        let xd = ((SAMPLE_SCREEN_WIDTH / 2) as f32 * (1.0 - cos_angle)) as i32;
        let yd = ((SAMPLE_SCREEN_HEIGHT / 2) as f32 - sin_angle * (SAMPLE_SCREEN_WIDTH / 2) as f32)
            as i32;
        ...
        let display_flag: BackgroundFlag = unsafe { std::mem::transmute(flag_byte) };
...
impl NoiseSample {
        ...
        for y in 0..2 * SAMPLE_SCREEN_HEIGHT {
            for x in 0..2 * SAMPLE_SCREEN_WIDTH {
                ...
                let value = match self.func {
                    NoiseFunction::Perlin => self.noise.get_ex(&mut coords, NoiseType::Perlin),
                    NoiseFunction::Simplex => self.noise.get_ex(&mut coords, NoiseType::Simplex),
                    NoiseFunction::Wavelet => self.noise.get_ex(&mut coords, NoiseType::Wavelet),
                    NoiseFunction::FbmPerlin => {
                        self.noise
                            .get_fbm_ex(&mut coords, self.octaves, NoiseType::Perlin)
                    }
                    NoiseFunction::TurbulencePerlin => {
                        self.noise
                            .get_turbulence_ex(&mut coords, self.octaves, NoiseType::Perlin)
                    }
                    NoiseFunction::FbmSimplex => {
                        self.noise
                            .get_fbm_ex(&mut coords, self.octaves, NoiseType::Simplex)
                    }
                    NoiseFunction::TurbulenceSimplex => {
                        self.noise
                            .get_turbulence_ex(&mut coords, self.octaves, NoiseType::Simplex)
                    }
                    NoiseFunction::FbmWavelet => {
                        self.noise
                            .get_fbm_ex(&mut coords, self.octaves, NoiseType::Wavelet)
                    }
                    NoiseFunction::TurbulenceWavelet => {
                        self.noise
                            .get_turbulence_ex(&mut coords, self.octaves, NoiseType::Wavelet)
                    }
                };
    ...
    fn draw_rectangle(&self, console: &mut Offscreen) {
        console.set_default_background(Color::GREY);
        let height = if self.func as u32 <= NoiseType::Wavelet as u32 {
            10
        } else {
            13
        };
        ...
        iterate_map(&mut |x, y, c| {
            ...
            if !visible {
                ...
                if !self.torch {
                    ...
                    let r = (x as f32 - self.px as f32 + dx) * (x as f32 - self.px as f32 + dx)
                        + (y as f32 - self.py as f32 + dy) * (y as f32 - self.py as f32 + dy);
...
impl Render for FovSample {
    ...
    fn render(
        ...
        if self.recompute_fov {
            ...
            self.map
                .compute_fov(self.px, self.py, radius, self.light_walls, self.algorithm);
        }
...
impl<'a> PathSample<'a> {
        ...
        F: Fn(&mut PathSample) -> (),
    {
        ...
        if SMAP[self.dy as usize]
            .chars()
            .nth(self.dx as usize)
            .unwrap()
            == ' '
        {
            self.recalculate_path = true;
...
impl<'a> Render for PathSample<'a> {
    ...
    fn render(
        ...
        if let Some((_, Event::Key(key))) = event {
            match key {
                ...
                Key { printable: 'k', .. } | Key { printable: 'K', .. }
                    if self.dy < SAMPLE_SCREEN_HEIGHT - 1 =>
                {
                    self.handle_event(console, &mut |s| s.dy += 1)
                }
                ...
                Key { printable: 'l', .. } | Key { printable: 'L', .. }
                    if self.dx < SAMPLE_SCREEN_WIDTH - 1 =>
                {
                    self.handle_event(console, &mut |s| s.dx += 1)
                }
                Key {
        ...
        if let Some((_, Event::Mouse(state))) = event {
            ...
            if mx >= 0
                && mx < SAMPLE_SCREEN_WIDTH
                && my >= 0
                && my < SAMPLE_SCREEN_HEIGHT
...
impl<'a> BspSample<'a> {
    ...
    fn rebuild(&mut self) {
        self.bsp
            .resize(0, 0, SAMPLE_SCREEN_WIDTH, SAMPLE_SCREEN_HEIGHT);
        ...
        self.bsp
            .traverse(TraverseOrder::InvertedLevelOrder, |mut node| {
                self.visit(&mut node, &mut map)
            });
        self.map = map;
        self.generate = false;
        self.refresh = false;
    ...
    fn visit_leaf(&self, node: &mut Bsp, map: &mut CharMap) {
        ...
        if max_x == SAMPLE_SCREEN_WIDTH - 1 {
            max_x -= 1
        }
        if max_y == SAMPLE_SCREEN_HEIGHT - 1 {
            max_y -= 1
        }
        if self.random_room {
        ...
        match event {
            Some((_, Event::Mouse(mouse))) => {
                if mouse.lbutton_pressed {
                    self.left_button = !self.left_button;
                }
                if mouse.mbutton_pressed {
                    self.middle_button = !self.middle_button;
                }
                if mouse.rbutton_pressed {
                    self.right_button = !self.right_button;
                }
...
impl NameSample {
    fn new() -> Self {
        ...
        let entries = read_dir("data/namegen")
            .ok()
            .expect("Could not read data/namegen");
...
fn main() {
    ...
    let mut samples = vec![
        MenuItem::new("  True colors      ", &mut colors),
    ...
    if options.fullscreen_width > 0 {
        system::force_fullscreen_resolution(options.fullscreen_width, options.fullscreen_height);
    }
...
fn print_help_message(root: &mut Root) {
    ...
    root.print_ex(
        ...
        BackgroundFlag::None,
        TextAlignment::Right,
        format!("elapsed {:8}ms {:4.2}s", millis, millis as f32 / 1000.0),
    );

    root.print(
        2,
        47,
        format!("{}{} : select a sample", chars::ARROW_N, chars::ARROW_S),
    );
